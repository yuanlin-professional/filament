...
`CommandBufferQueue.cpp` 实现了命令缓冲区队列，是 Filament 多线程渲染架构中连接应用线程（生产者）和渲染线程（消费者）的核心组件。负责管理环形缓冲区中命令数据的提交、等待和释放。

## 核心类/结构体

### `CommandBufferQueue`
- 头文件位于 `private/backend/CommandBufferQueue.h`。
- 内部持有 `CircularBuffer` 实例，管理命令缓冲区的生命周期。
- **flush()**: 应用线程调用，将当前命令缓冲区提交到执行队列。添加 `NoopCommand` 终止标记，更新可用空间，在空间不足时阻塞等待。
- **waitForCommands()**: 渲染线程调用，阻塞等待直到有命令可执行或收到退出请求。
- **releaseBuffer()**: 渲染线程执行完命令后调用，归还缓冲区空间。
- **requestExit()**: 请求退出，唤醒等待中的渲染线程。
- **setPaused()** / **isPaused()**: 暂停/恢复命令处理。

## 关键实现逻辑

- 使用条件变量（`mCondition`）实现生产者-消费者同步。
- `flush()` 中检测缓冲区溢出：如果已用空间超过可用空间，说明命令流已损坏，触发不可恢复的错误。
- 当可用空间不足 `mRequiredSize` 时，应用线程阻塞等待渲染线程释放空间。
- 暂停状态下 `flush()` 会死锁检测：如果缓冲区满且渲染线程被暂停，直接中止以避免死锁。
- `mCommandBuffersToExecute` 使用 `std::move` 传递给消费者，避免数据拷贝。

## 依赖关系

- `private/backend/CircularBuffer.h` - 环形缓冲区
- `private/backend/CommandStream.h` - `NoopCommand` 终止命令
- `utils::Mutex` / `utils::Condition`（通过 std） - 同步原语
